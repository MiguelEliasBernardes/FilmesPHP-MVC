<ul class="grid grid-cols-4 gap-6">
    <?php foreach ($movies as $movie): ?>
        <li id="<?= $movie['id_movie'] ?>" onmouseleave="RemoveDescription(this)" onmouseover="displayDescription(this)" class=" bg-[url(<?= $movie['image'] ?>)] bg-cover bg-center h-96 rounded-xl movie group ">
            <div class="bg-gradient-to-b from-transparent to-(--gray-200) flex flex-col justify-between h-full rounded-b-lg transition-all duration-200 hover:bg-(--gray-200)/25">
                ...
                <div class="pl-4 pb-4 ">
                    ...
                    <p class="max-h-16 h-0 opacity-0 translate-y-5 overflow-hidden description  invisible duration-200 font-(family-name:--nunito) text-(--gray-600) text-sm group-hover:visible group-hover:h-16  transition-all group-hover:translate-y-0 group-hover:opacity-100"><?= $movie['description'] ?></p>
                </div>
            </div>
        </li>
    <?php endforeach ?>
